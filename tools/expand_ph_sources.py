"""Append 400 sweets, bakery, spreads and supporting references; preserve prior foods."""
import argparse
import hashlib
import json
import re
...
BATCH='2026-09-17-ph-restaurants-sources'
from select_ph_sources import plan
...
def apply(selected,excluded,fnri,usda):
    ...
    for i,entry in enumerate(selected):
        ...
        if (i+1)%100==0:
            foods=tables['foods.csv'];assert foods[:1809]==original
    ...
    assert len(tables['foods.csv'])==1809+len(selected)
    ...
    tables['expansion-ph-restaurants-sources.csv']=members;tables['expansion-ph-restaurants-exclusions.csv']=excluded
    ...
    summary=dict(expansion_id=BATCH,added=len(selected),fnri=sum(e['source']=='FNRI' for e in selected),usda=sum(e['source']=='USDA' for e in selected),total=len(tables['foods.csv']),original_records_preserved=1809,
        household_portions_added=household,estimates_added=0,
        added_fnri_statuses=dict(Counter(r['data_status'] for r in tables['foods.csv'][1809:] if 'FNRI' in r['source_name'])))
...
if __name__=='__main__':
    ...
    if (DB/(BATCH+'.json')).exists():print('Batch already applied. No changes.')
        ...
        if args.apply:apply(selected,excluded,fnri,usda)
            ...
            print(json.dumps(dict(selected=len(selected),fnri=sum(e['source']=='FNRI' for e in selected),excluded_count=len(excluded)),indent=2))
