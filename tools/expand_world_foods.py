"""Append 54 foods with frozen sources, recipe assumptions and preservation checks."""
...
from world_food_sources import plan
from world_food_plans import plans,SOURCES,RESTAURANT_INGREDIENTS,REGIONAL_INGREDIENT_IDS,EXTRA_INGREDIENTS
from resolve_food import FoodCatalog

DATE='2026-09-18'
BATCH=DATE+'-world-foods-54'
...
def build(apply=False):
    ...
    assert len(tables['foods.csv'])==2819,'Review changed baseline before applying'
    ...
    for index,p in enumerate(plans(),1):
        fid=food_id();eid=f'EST_WORLD_20260918_{index:03d}_V1';parts=[];urls=[p['url']]
        ...
        review.append(dict(food_id=fid,name=name,fnri_related_ids=';'.join(fnri_matches),existing_related_food_ids=';'.join(related),decision='Distinct brand or specified cuisine recipe model; existing source-qualified foods remain unchanged.',nutrition_status='NOT_VERIFIED_EXPLICIT_ESTIMATE',identity_source_url=p['url']))
    assert len(members)==54 and len(tables['foods.csv'])==2873
    assert len({r['food_id'] for r in tables['foods.csv']})==2873
    for n,rows in tables.items():assert rows[:len(prior[n])]==prior[n],n
    summary=dict(batch=BATCH,added=54,total_foods=2873,prior_food_rows_preserved=2819,fnri=0,usda=20,dish_estimates=10,restaurant_estimates=24,by_group=dict(Counter(r['group'] for r in members)),household_portions_added=household,ingredient_rows=len(components),baseline_sha256={n:hashlib.sha256(raw).hexdigest() for n,raw in baseline.items()},deployed=False)
    ...
    for n,rows in tables.items():
        temp=DB/(n+'.world.tmp');write_csv(temp,rows);temp.replace(DB/n)
    write_csv(DB/'world-expansion-54.csv',members)
    write_csv(DB/'world-estimate-components.csv',components)
    write_csv(DB/'world-identity-review.csv',review)
    write_csv(DB/'world-reference-review.csv',excluded)
    audit=[dict(group=g,url=url,reviewed_date=DATE,role='IDENTITY_ONLY_NOT_NUTRITION',access_note='Official delivery search-index listing reviewed; direct open failed.' if g=="Royal Indian Curry House by Swaadisht" else 'Primary menu/recipe/tourism identity source reviewed via web search/open. Historic descriptions do not establish current availability. All recipe weights independently assumed, not copied from source.',nutrition_policy='Ingredient masses assumed; no measured branded/regional nutrition or household weight.') for g,url in SOURCES.items()]
    write_csv(DB/'world-online-source-audit.csv',audit)
