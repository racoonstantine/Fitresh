# Fitresh — development history (archive)

Long, dated notes on how each feature was built and fixed. **Read this only when you need the
reasoning behind a specific past change.** It is not kept up to date: early sections still say the
frontend is "one file `public/index.html`" (it is now split, see `docs/handoff.md`), and
"Full Circle" is the app's earlier name. Current guide: `README.md` and `docs/handoff.md`.
